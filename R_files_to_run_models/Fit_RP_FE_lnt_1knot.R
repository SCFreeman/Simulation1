...
# Load model
source("Function_RP_FE_lnt_1knot.R")

# Empty data frame
rp_fe_1knot <- data.frame(i=numeric(df_num), dgm=numeric(df_num), knots=numeric(df_num),
...
foreach(i=1:df_num) %do% {
  print(i)
  rp_fe_1knot[i,] <- royston_parmar_fe_lnt_1knot(df=df.list[[i]], num.sims=num.sims, burn.in=burn.in)
}

write.csv(rp_fe_1knot, "Results_RP_FE_lnt_1knot.csv")