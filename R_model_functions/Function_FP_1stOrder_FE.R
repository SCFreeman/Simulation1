...
fp_firstorder_fe <- function(data, P1, num.sims, burn.in, maxt){
  ...
  bugs.object<-try(R2OpenBUGS::bugs(data=bugs_data, inits=inits, 
                                ...
                                bugs.seed=3, 
                                working.directory="/lustre/ahome3/s/scf20/Documents/Simulation/Scenario1/FP_1",
                                debug=F))
  
  if(class(bugs.object)=="try-error"){
    print("error fitting in OpenBUGS")
    fp_results <- c(i=i, dgm=NA, p1=P1,
  ...
  fp_results <- data.frame(i=data$i[1], dgm=data$dgm[1], p1=P1,
