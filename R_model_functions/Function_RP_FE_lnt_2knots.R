# Royston-Parmar model in R

royston_parmar_fe_lnt_2knots <- function(df, num.sims, burn.in){
  ...
  # Need to calculate 33rd and 67th perentiles of uncensored survival times for each trial
  
  knots <- data.frame(trialid=1:num_studies, k1=numeric(num_studies), k2=numeric(num_studies), 
                      k3=numeric(num_studies), k4=numeric(num_studies))
  for (i in 1:num_studies) {
    k <- quantile(data$lnt[data$died==1 & data$trialid==i], c(0, 0.33, 0.67, 1))
    knots[i,2] <- k[1]
    knots[i,3] <- k[2]
    knots[i,4] <- k[3]
    knots[i,5] <- k[4]
  ...
  bugs_data <- list(d=data$died, trt=data$treat, trtlnt=data$trtlnt, u0=data$u0, u1n=data$u1n, 
                    ...
                    w1nw1n=df4$u1nu1n[4], meanw2=df5$mean[4], sdw2=df5$sd[4], nt=2)
  ...
  bugs.object<-try(R2OpenBUGS::bugs(data=bugs_data, inits=inits, 
                                ...
                                model.file="Model_RP_FE_lnt_2knots.txt", clearWD=F, 
                                summary.only=FALSE, n.iter=(num.sims+burn.in),
                                n.burnin=burn.in, n.chains=3, 
                                bugs.seed=3, 
                                working.directory="/lustre/ahome3/s/scf20/Documents/Simulation/Scenario1/RP_lnt_2knots",
                                debug=F))
  
  if(class(bugs.object)=="try-error"){
    print("error fitting in OpenBUGS")
    rp_results <- c(i=i, dgm=NA, knots=2,
  ...
  rp_results <- data.frame(i=data$i[1], dgm=data$dgm[1], knots=2,
