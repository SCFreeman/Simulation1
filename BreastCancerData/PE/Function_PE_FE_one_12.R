# Fit piecewise model in OpenBUGS - one cut point at 12 months

piecewise_one_cut_try12 <- function(data, num.sims, burn.in, cutpoint){
  ...
  bugs.object<-try(R2OpenBUGS::bugs(data=ma_data, inits=inits, 
                                    ...
                                    bugs.seed=3, 
                                    working.directory="/alice-home/2/s/scf20/Documents/Example_HPC/PE_cut12", 
  ...
  if(class(bugs.object)=="try-error"){
    ...
    pe_results <- data.frame(rmstD=results[1,5], 
                             rmstD_lci=results[1,3], rmstD_uci=results[1,7], rmstD_sd=results[1,2],
